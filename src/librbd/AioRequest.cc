...
namespace librbd
{
    ...
    int AioRead::send()
    {
        ...
        /*# new librados::AioCompletionImpl *c 并设置 rados_callback_t callback_complete = rados_req_cb
        void *callback_complete_arg = this (librbd::AioWrite)
        然后new AioCompletion(c)  librados::AioCompletion  AioCompletionImpl *pc = c */
        librados::AioCompletion *rados_completion =
            librados::Rados::aio_create_completion(this, rados_req_cb, NULL);
        int r;
        librados::ObjectReadOperation op; //#新建一个op ,write是作为AbstractWrite的属性  librados::ObjectWriteOperation m_write (一个是属性，一个是临时变量，有什么区别)
        int flags = m_ictx->get_read_flags(m_snap_id);
        if (m_sparse)
        {
            //# librados::ObjectReadOperation::sparse_read
        ...
        //# librados::ObjectOperation::set_op_flags2    librados.cc:115
        op.set_op_flags2(m_op_flags);

        //# librados::IoCtx::aio_operate  librados.cc:1371
    ...
    {
        // send 0x1ff91b0 rb.0.1011.6b8b4567.000000000000 0~4194304
    ...
    {
        //log: send_write 0x1ff91b0 rb.0.1011.6b8b4567.000000000000 0~4194304
        ...
        //# 注册rados相关的回调函数 rados_req_cb (读写都一样)
        //# librbd::C_AioWrite *req_comp  ->  librbd::AioRequest Context *m_completion

        /*# new librados::AioCompletionImpl *c 并设置 rados_callback_t callback_safe = rados_req_cb
        void *callback_safe_arg = this (librbd::AioWrite)
        然后new AioCompletion(c)  librados::AioCompletion  AioCompletionImpl *pc = c */
        librados::AioCompletion *rados_completion =
            librados::Rados::aio_create_completion(this, NULL, rados_req_cb); //# read是 (this, rados_req_cb, NULL)
        //# IoCtx::aio_operate librados.cc 1330
        int r = m_ictx->data_ctx.aio_operate(m_oid, rados_completion, &m_write,
                                             m_snap_seq, m_snaps);
        assert(r == 0);
        //# 调用 librados::AioCompletion  AioCompletionImpl *pc ->release()
        rados_completion->release();
    }
    ...
    {
        //# 添加OP CEPH_OSD_OP_SETALLOCHINT
        wr->set_alloc_hint(m_ictx->get_object_size(), m_ictx->get_object_size());
        //# 添加OP CEPH_OSD_OP_WRITE
        wr->write(m_object_off, m_write_data);
        //# 设置最后一个OP,这里是WRITE的标志
