...
namespace librbd
{
                      ...
                      bool *old_format, uint64_t *size)
    {
        ...
        //# log: detect format of volume-1 : old
...
#define READ_SIZE 4096
    ...
    //# io_ctx=..., imgname=0x2b95e10 "volume-3", bid=174157, size=2147483648, order=22
    ...
    //# 旧格式没有条带
    //# io_ctx=..., imgname=0x2b95e10 "volume-3", size=2147483648, order=0x7fffffffdb44
    int create(librados::IoCtx& io_ctx, const char *imgname, uint64_t size,
               int *order)
    {
        CephContext *cct = (CephContext *)io_ctx.cct();
        bool old_format = cct->_conf->rbd_default_format == 1;//# 如果格式为1为旧格式，默认为2
        uint64_t features = old_format ? 0 : cct->_conf->rbd_default_features;//# 旧格式特性为0，默认为3
               ...
               uint64_t stripe_unit, uint64_t stripe_count)
    {
        ...
        //# 判段image是否已存在，两个格式都要判断
        int r = detect_format(io_ctx, imgname, NULL, NULL);
        //#  r如果不是ENOENT，表示image已经存在了
        if (r != -ENOENT)
        {
            if (r)
            {
    ...
    {
        //# log: open_image: ictx = 0x1a7df80 name = 'volume-1' id = '' snap_name = ''
    ...
    {
        utime_t start_time, elapsed;
        //log: write 0x21c34f0 off = 0 len = 8388608
        ...
        Context *ctx = new C_SafeCond(&mylock, &cond, &done, &ret);
        //# rbd 回调相关，回调函数rbd_ctx_cb (读写是一样的)
    ...
    void rados_req_cb(rados_completion_t c, void *arg)
    {
        ...
        //# rados_aio_get_return_value 获取返回值
    ...
    {
        CephContext *cct = ictx->cct;
        //log: aio_write 0x21c34f0 off = 0 len = 8388608 buf = 0x7f45cf7ff010
        ...
        uint64_t overlap = 0;
        {
            ...
            //# {seq = {val = 12}, snaps = std::vector of length 1, capacity 1 = {{val = 12}}}
            snapc = ictx->snapc;
            ictx->parent_lock.get_read();
            //获取快照 parent的overlap(也就是从哪个点开始新建快照，如果没有快照，overlap就是0)
        ...
        /*#
        print p
        $1 = {oid = {name = "rb.0.1011.6b8b4567.", '0' <repeats 12 times>}, objectno = 0, offset = 0, length = 4194304, truncate_size = 0, oloc = {
        pool = 1, key = "", nspace = "", hash = -1}, buffer_extents = std::vector of length 1, capacity 1 = {{first = 0, second = 4194304}}}
        */
        for (vector<ObjectExtent>::iterator p = extents.begin(); p != extents.end(); ++p)
        {
            //# log: oid rb.0.1011.6b8b4567.000000000000 0~4194304 from [0,4194304]
            //# log: oid rbd_data.25a126b8b4567.0000000000000000 0~4194304 from [0,1048576,4194304,1048576,8388608,1048576,12582912,1048576]
            ...
            else
            {
                ...
                //# 获取条带在文件(卷)中的位置
                //# std::vector of length 4, capacity 5 = {{first = 0, second = 1048576}, {first = 4194304, second = 1048576}, {first = 8388608, second = 1048576}, {first = 12582912, second = 1048576}}
                //# std::vector of length 4, capacity 5 = {{first = 1048576, second = 1048576}, {first = 5242880, second = 1048576}, {first = 9437184, second = 1048576}, {first = 13631488,  second = 1048576}}
                Striper::extent_to_file(ictx->cct, &ictx->layout,
                                        p->objectno, 0, ictx->layout.fl_object_size,
                                        objectx);
                //# 获取覆盖 parent 内容的大小
                //# std::vector of length 0, capacity 5
                uint64_t object_overlap = ictx->prune_parent_extents(objectx, overlap);

                //# librbd::C_AioWrite *req_comp  ->  librbd::AioRequest Context *m_completion
                AioWrite *req = new AioWrite(ictx, p->oid.name, p->objectno, p->offset,
                                             objectx, object_overlap,
                                             bl, snapc, snap_id, req_comp);
                //# 每个分片自己send一次，回调请求加一，要所有分片函数都返回才调用回调函数
                c->add_request();
                 ...
                 char *buf, bufferlist *pbl, AioCompletion *c, int op_flags)
    {
        ...
        uint64_t buffer_ofs = 0;
        {
            ...
            snap_id = ictx->snap_id;
            //# {seq = {val = 0}, snaps = std::vector of length 0, capacity 0}
            snapc = ictx->snapc;
