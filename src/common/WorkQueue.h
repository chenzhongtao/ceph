...
class ThreadPool : public md_config_obs_t
{
    ...
    int _draining;
    Cond _wait_cond;
    int ioprio_class, ioprio_priority; //# -1 -1
...
public:
    ...
    protected:
        virtual void _process(const list<T*> &)
        {
            assert(0);
        }
...
private:
    //# 由多种工作队列组成, 不同的工作队列都是继承WorkQueue_
    vector<WorkQueue_*> work_queues;
    int last_work_queue; //# 记录上一次是哪个工作队列,就是上面vector的index,多个线程共用
    ...
    set<WorkThread*> _threads;  //# 存放工作线程
    list<WorkThread*> _old_threads;  ///< need to be joined
    int processing;  //# 正在处理的线程数
