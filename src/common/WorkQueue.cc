...
ThreadPool::ThreadPool(CephContext *cct_, string nm, int n, const char *option)
    : cct(cct_), name(nm),  //# "FileStore::op_tp"
      lockname(nm + "::lock"),
...
{
    if (option) {
        _thread_num_option = option;//# "filestore_op_threads"
        // set up conf_keys
...
void ThreadPool::worker(WorkThread *wt)
{
    ...
    while (!_stop) {
        ...
        join_old_threads();
        if (_threads.size() > _num_threads) { //# 线程过多,退出线程
            ldout(cct,1) << " worker shutting down; too many threads (" << _threads.size() << " > " << _num_threads << ")" << dendl;
            _threads.erase(wt);
            _old_threads.push_back(wt);//# 放到旧线程中
            break;
        }

        if (!_pause && !work_queues.empty()) { //# 线程没有暂停,并且工作队列不为空
            WorkQueue_* wq;
            ...
            while (tries--) {
                ...
                if (item) {
                    processing++;//# 正在工作的线程数加1
                    ldout(cct,12) << "worker wq " << wq->name << " start processing " << item
                                  << " (" << processing << " active)" << dendl;
                    TPHandle tp_handle(cct, hb, wq->timeout_interval, wq->suicide_interval);
                    tp_handle.reset_tp_timeout();
                    _lock.Unlock();
                    wq->_void_process(item, tp_handle);//# 线程处理
                    _lock.Lock();
                    wq->_void_process_finish(item); //# 结尾操作
                    processing--; //# 正在工作的线程数减1
                    ldout(cct,15) << "worker wq " << wq->name << " done processing " << item
                                  << " (" << processing << " active)" << dendl;
                    if (_pause || _draining)
                        _wait_cond.Signal();
                    did = true;
                    break;//# 一个线程处理一个就先退出 while (tries--)
                }
            }
            if (did)
                continue;
        ...
        cct->get_heartbeat_map()->reset_timeout( //# 重新设置超时时间,上一个线程处理过久这里会打印
            hb,
            cct->_conf->threadpool_default_timeout,  //# 60秒超时
            0);
        _cond.WaitInterval(cct, _lock,
                           utime_t(
                               cct->_conf->threadpool_empty_queue_max_wait, 0)); //# 2
    }
    ldout(cct,1) << "worker finish" << dendl;

    cct->get_heartbeat_map()->remove_worker(hb); //# 移除heartbeat_map worker
...
void ThreadPool::pause()
{
    ...
    _pause++;
    while (processing)
        _wait_cond.Wait(_lock); //# 等待正在运行的线程运行完
    _lock.Unlock();
