...
//# raw数据的初始地址加上偏移量(一个raw可能被多个ptr使用，所以ptr有个_off属性，表示其起始地址是在raw起始地址的那里)
...
void buffer::ptr::append(const char *p, unsigned l)
{
    ...
    //# c_str() raw数据的初始地址加上偏移量(一个raw可能被多个ptr使用，所以ptr有个_off属性，表示其起始地址是在raw起始地址的那里)
    //# 这里的_len是这个ptr在这个raw的已使用长度
...
//# 从迭代器当前位置拷贝长度为len的数据到 dest(只需拷贝ptr)
...
bool buffer::list::contents_equal(ceph::buffer::list& other)
{
    ...
    if (true)
    {
        //# 其实iterator就是某总类型的指针 这里是 ptr*  ptr内部定义迭代器是做什么的
...
void buffer::list::append(const char *data, unsigned len)
{
    while (len > 0)
    {
        ...
        if (gap > 0)
        {
            ...
            //# 把新增的这段添加到list中 std::list<ptr> _buffers;
        ...
        append_buffer = create_page_aligned(alen);
        //# 当前的raw还没被使用，使用长度为0
        append_buffer.set_length(0);   // unused, so far. 初始化时长度等于分配的内存大小
    }
}
...
void buffer::list::append(const ptr& bp, unsigned off, unsigned len)
{
    ...
    if (!_buffers.empty())
    {
        //# 获取最后一个ptr
        ptr &l = _buffers.back();
        //# 如果使用的是同个raw ,并且地址连续，添加在上一个的后面就可以
