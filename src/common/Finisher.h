...
class Finisher
{
    ...
    //# 如果队列对应的Context为NULL,表示对应的Context是存在finisher_queue_rval,所以只需遍历finisher_queue
...
public:
    ...
    void queue(Context *c, int r = 0)
    {
        ...
        if (finisher_queue.empty()) {
            finisher_cond.Signal();//# 不为空时线程没有暂停等待,所以不用叫醒
        }
        if (r) {
        ...
        finisher_lock.Unlock();//# 等待解锁之后,那边的wait才会继续运行,所以finisher_cond.Signal()在前面也可以
    }
