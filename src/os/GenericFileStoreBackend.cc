...
int GenericFileStoreBackend::detect_features()
{
    ...
    if (!m_filestore_fiemap) {//# 默认fiemap是关闭的
    ...
    if (!m_filestore_seek_data_hole) {  //# 默认是关闭的
    ...
    if (!m_filestore_splice) {
        ...
        if ((r = pipe(pipefd)) < 0)
            ...
            lseek(fd, 0, SEEK_SET);
            r = splice(fd, &off_in, pipefd[1], NULL, 10, 0);//# 从由fd指向的文件读取数据到由pipefd[1]指向的内核缓冲区,除在内核空间和用户空间之间的内存拷贝的开销
            if (!(r < 0 && errno == EINVAL)) {
