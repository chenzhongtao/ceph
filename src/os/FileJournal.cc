...
int FileJournal::_open(bool forwrite, bool create)
{
    ...
    if (S_ISBLK(st.st_mode)) { //# 是否是块设备
        ret = _open_block_device();
    } else {
        if (aio && !force_aio) { //# 不是块设备不能使用aio要使用force_aio
...
int FileJournal::_open_block_device()
{
    ...
    if (g_conf->journal_discard) { //# 还不知道什么功能
                            ...
                            bool create)
{
    ...
    if (create && (oldsize < conf_journal_sz)) {
        ...
        ret = ::ftruncate(fd, newsize); //# 改变文件大小
        if (ret < 0) {
            int err = errno;
        ...
        ret = ::posix_fallocate(fd, 0, newsize); //# 分配大小
        if (ret) {
...
# error "Journal pre-allocation not supported on platform."
    ...
    if (create && g_conf->journal_zero_on_create) { //# 所有数据初始化为0
...
int FileJournal::create()
{
    ...
    header.start = get_top(); //# 开始写日志的位置, header保存在前面的block_size中
    header.start_seq = 0;  //# 现在的seq号
    ...
    memset(buf, 0, block_size);//# 写第二个4K,全为0 
    ...
    needed_space = ((int64_t)g_conf->osd_max_write_size) << 20; //# 默认90M
...
//# 把日志读到这个指定的seq,指定为commit_op_seq,所以会把已经写的日志但还不能覆盖的日志读到 journalq,从header.start开始读
int FileJournal::open(uint64_t fs_op_seq)
{
    ...
    // 0 if the start is 0
    //#  fs_op_seq  = cat /current/commit_op_seq ,已经写入磁盘的seq
...
int FileJournal::read_header(header_t *hdr) const
{
    ...
    if (bp.length() != (size_t)r) {
        ...
        bpdata += r;
        memset(bpdata, 0, bp.length() - r); //# 后面补0
    }
...
int FileJournal::check_for_full(uint64_t seq, off64_t pos, off64_t size)//# 1 4096 8192
{
    ...
    //# 日志是个环,pos == header.start表示空,pos == header.start-1 表示满了
    off64_t room; //# 计算还可以写的大小
    if (pos >= header.start)
        room = (header.max_size - pos) + (header.start - get_top()) - 1; //# pos之后可以写,header.start之前可以写(header.start 和 pos之间是还不可以覆盖的日志部分)
    else
    ...
    if (room >= size) {
        ...
        if (pos + size > header.max_size) //# 是否需要从前面往回写
            must_write_header = true;
        return 0;
    }
...
int FileJournal::prepare_multi_write(bufferlist& bl, uint64_t& orig_ops, uint64_t& orig_bytes)
{
    ...
    int eleft = g_conf->journal_max_write_entries; //# 100
    unsigned bmax = g_conf->journal_max_write_bytes; //# 32M
    ...
    while (!writeq_empty()) {
        ...
        if (r == -ENOSPC) {
            ...
            return -ENOSPC;  // hrm, full on first op  第一个操作就满了
        }

        if (eleft) {//# 每合并一项就减一,最多合并100项
            if (--eleft == 0) {
                dout(20) << "prepare_multi_write hit max events per write " << g_conf->journal_max_write_entries << dendl;
                break;
            }
        }
        if (bmax) { //# 合并多个写长度超过32M就先退出循环
            if (bl.length() >= bmax) {
...
*/
//# 把completions队列中,seq小于等于我的,出列,放到finisher_queue,执行回调函数
...
int FileJournal::prepare_single_write(bufferlist& bl, off64_t& queue_pos, uint64_t& orig_ops, uint64_t& orig_bytes)
{
    ...
    unsigned head_size = sizeof(entry_header_t);//# 有两种头部 entry_header_t 和 header_t
    off64_t base_size = 2*head_size + ebl.length(); //# 头部需要保存两份 ? 看 do_read_entry

    int alignment = next_write.alignment; // we want to start ebl with this alignment ,想让数据是alignment对齐的
    unsigned pre_pad = 0;
    if (alignment >= 0)  //# -1表示不用对齐,这个对齐是说日志中数据部分的对齐,因为数据前面有一个头部,所以要减去一个头部先
		//# 4056   (0-40) & (4095)   //#最浪费空间 
    ...
    // add it this entry 添加entry头部,有两种头部
    entry_header_t h;
    memset(&h, 0, sizeof(h));
    h.seq = seq;
    h.pre_pad = pre_pad;
    ...
    bl.append((const char*)&h, sizeof(h));//# 前面写头部
    if (pre_pad) {
        bufferptr bp = buffer::create_static(pre_pad, zero_buf);//# 补零
        bl.push_back(bp);
    }
    bl.claim_append(ebl, buffer::list::CLAIM_ALLOW_NONSHAREABLE); // potential zero-copy

    if (h.post_pad) {//# 后面补零对齐
        bufferptr bp = buffer::create_static(post_pad, zero_buf);
        bl.push_back(bp);
    }
    bl.append((const char*)&h, sizeof(h));//# 后面写头部 ?
...
void FileJournal::do_write(bufferlist& bl)
{
    ...
    if (pos + bl.length() > header.max_size) {
        ...
        orig_pos = pos;
        if (write_bl(pos, second)) {//# 写后面部分
            derr << "FileJournal::do_write: write_bl(pos=" << orig_pos
                 << ") failed" << dendl;
            ceph_abort();
        }
        orig_pos = first_pos;
        if (write_bl(first_pos, first)) {//# 写前面部分
            derr << "FileJournal::do_write: write_bl(pos=" << orig_pos
                 << ") failed" << dendl;
            ceph_abort();
        }
                               ...
                               Context *oncommit, TrackedOpRef osd_op)
{
...

    {
        ...
        completions.push_back(  //#　committed_thru　会用到
            completion_item(
...
void FileJournal::committed_thru(uint64_t seq)
{
    ...
    // completions!
    {
		...
		//# plug_journal_completions 初始为false FULL_WAIT时为true
		//# seq >= header.start_seq 表示开始刷日志了,不用阻塞日志回调
	...
	//# 更新 header.start 和 header.start_seq
    if (!journalq.empty()) {
...
int FileJournal::make_writeable()
{
    ...
    if (read_pos > 0)
        ...
        write_pos = get_top();
    read_pos = 0; //# 把read_pos置0,写日志的过程不会改read_pos
...
//# 从read_pos开始读取一项,如果传进来的next_seq大于读取到的seq,返回false,否则,更新read_pos和返回这个seq的号,和这个seq的数据
//# bl是数据部分,不包括头部和补齐部分
    ...
    bool *corrupt)
{
    ...
    stringstream ss;
	//# 根据位置pos,读取一项, 返回下一项的next_pos, 这一项的数据,seq,信息,头部
    read_entry_result result = do_read_entry(
                                   pos,
                                   &next_pos,
                                   &bl,
    ...
    if (seq && seq < header.committed_up_to) {
        ...
        derr << "Unable to read past sequence " << seq
             << " but header indicates the journal has committed up through "
             << header.committed_up_to << ", journal is corrupt" << dendl;
        if (g_conf->journal_ignore_corruption) {//# false
            if (corrupt)
                *corrupt = true;
            return false;
        } else {
...
//# 根据位置init_pos,读取一项, 返回下一项的pos, 这一项的数据,seq,信息,头部
    ...
    off64_t corrupt_at)
{
    ...
    buf[0]++;//# 修改数据,再写入 , 为什么不能直接使用fd,还要使用外面传来的wfd,打开都是同一个文件
