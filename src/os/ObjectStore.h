...
public:
    ...
    private:
        ...
        bool use_tbl;   //use_tbl for encode/decode 构造函数初始化为false 
        bufferlist tbl;
    ...
    private:
        ...
        Op* _get_next_op()
        {
            if (op_ptr.length() == 0 || op_ptr.offset() >= op_ptr.length()) {
                op_ptr = bufferptr(sizeof(Op) * OPS_PER_PTR);//# 重新分配
            }
            ...
            return reinterpret_cast<Op*>(p);  //# reinterpret_cast用来处理无关类型之间的转换
        }
                           ...
                           ThreadPool::TPHandle *handle = NULL)
    {
        ...
        return queue_transactions(osr, tls, op, handle);  //# FileStore::queue_transactions
    }
