...
FileStore::FileStore(const std::string &base, const std::string &jdev, osflagbits_t flags, const char *name, bool do_update) :
    ...
    op_wq(this, g_conf->filestore_op_thread_timeout,
          g_conf->filestore_op_thread_suicide_timeout, &op_tp),  //# 60  180
    logger(NULL),
...
void FileStore::new_journal()
{
    if (journalpath.length()) {
        ...
        journal = new FileJournal(fsid, &finisher, &sync_cond, journalpath.c_str(),
                                  m_journal_dio, m_journal_aio, m_journal_force_aio); //# true true false
        if (journal)
...
//# 创建文件系统，也就是生成 basedir 目录下的文件和目录
int FileStore::mkfs()
{
    int ret = 0;
    ...
    // version stamp
    ret = write_version_stamp(); //# hexdump /tmp/osd/store_version 写的是int的字节序，cat 看不出
    if (ret < 0) {
    ...
    // superblock
    superblock.omap_backend = g_conf->filestore_omap_backend; //# "leveldb"
    ...
    // write initial op_seq
    {
        ...
        if (initial_seq == 0) {
            ...
            if (backend->can_checkpoint()) { //# xfs不能
                // create snap_1 too
...
int FileStore::mkjournal()
{
    ...
    if (journal) {
        ...
        if (ret < 0) {
            ret = journal->create(); //# FileJournal::create()
            if (ret)
...
int FileStore::read_op_seq(uint64_t *seq)
{
    ...
    *seq = atoll(s); //# 空的话，seq=0
    return op_fd;
}

//# 写seq /current/commit_op_seq
...
int FileStore::mount()
{
    ...
    if (!backend->can_checkpoint()) {
        ...
        // from it.
        int r = ::creat(nosnapfn, 0644);  //# current/nosnap
        if (r < 0) {
    ...
    if (journal) {
        if (!m_filestore_journal_writeahead &&
            ...
            if (m_filestore_journal_writeahead) //# 默认
    ...
    wbthrottle.start();
    sync_thread.create(); //# FileStore 中定义的类
...
//# 提交op入 OpSequencer::q 队列,  OpSequencer放到op_wq,线程池会在这里取
...
void FileStore::_do_op(OpSequencer *osr, ThreadPool::TPHandle &handle)
{
    wbthrottle.throttle();
    // inject a stall?
    if (g_conf->filestore_inject_stall) { //# 0 控制速度,睡几秒
        ...
        for (int n = 0; n < g_conf->filestore_inject_stall; n++)
            sleep(1);
        g_conf->set_val("filestore_inject_stall", "0");//# 设置为0
                                  ...
                                  ThreadPool::TPHandle *handle)
{
    ...
    if (g_conf->filestore_blackhole) {//# 把所有事务都丢弃,是不是为了测试
    ...
    assert(posr);
    if (posr->p) {
		//# 强制转换为OpSequencer类型, 继承 Sequencer_implRef
        osr = static_cast<OpSequencer *>(posr->p.get());//# 获取posr的一份引用
    ...
    if (journal && journal->is_writeable() && !m_filestore_journal_trailing) {
        Op *o = build_op(tls, onreadable, onreadable_sync, osd_op); //# 创建一个filestore op
        ...
        uint64_t op_num = submit_manager.op_submit_start(); //# 返回op编号,一个事务一个op
        o->op = op_num;//# 事务的op编号
		//# 是否把事务打印出来
        if (m_filestore_do_dump)
        ...
        if (m_filestore_journal_parallel) {
			...
			//# 把op号提交到 OpSequencer::jq列表
            osr->queue_journal(o->op); //# 把op编号添加到列表
        ...
        submit_manager.op_submit_finish(op_num); //# 更新op_submitted
        return 0;
    }
...
//# 日志写完,会调用这个回调函数 C_JournaledAhead::finisher
...
void FileStore::sync_entry()
{
    ...
    while (!stop) {
        ...
        max_interval.set_from_double(m_filestore_max_sync_interval); //# 5
        utime_t min_interval;
        min_interval.set_from_double(m_filestore_min_sync_interval); //# 0.01

        utime_t startwait = ceph_clock_now(g_ceph_context);
        if (!force_sync) {
            dout(20) << "sync_entry waiting for max_interval " << max_interval << dendl;
            sync_cond.WaitInterval(g_ceph_context, lock, max_interval); //# 等待一段时间
        } else {
            dout(20) << "sync_entry not waiting, force_sync set" << dendl; //# 强制sync,不用等待
        }
        ...
        if (force_sync) {
            ...
            if (woke < min_interval) { //# 太早被叫醒了,继续wait一段时间
