                        ...
                        Index *index)
{
    ...
    if (!((*index).index))
    {
        r = get_index(cid, index);//coll_t实际上代表了一个目录，目录中是对象的集合
        if (r < 0)
        {
...
FileStore::FileStore(const std::string &base, const std::string &jdev, osflagbits_t flags, const char *name, bool do_update) :
    ...
    op_wq(this, g_conf->filestore_op_thread_timeout,
          g_conf->filestore_op_thread_suicide_timeout, &op_tp),//上面初始化了threadpool对象op_tp，然后用来初始化workqueue对象
    logger(NULL),
...
#endif
    default:
        return new GenericFileStoreBackend(fs);//默认对应哪种文件系统?
    }
}

void FileStore::create_backend(long f_type)
{
    m_fs_type = f_type;//保存初始化的文件类型
...
int FileStore::mkfs()
{
    ...
    dout(1) << "mkfs in " << basedir << dendl; //创建了/mnt/osd0目录
    ...
    if (read_fsid(fsid_fd, &old_fsid) < 0 || old_fsid.is_zero())
    {
        ...
        fsid.print(fsid_str);
        strcat(fsid_str, "\n");
        ret = ::ftruncate(fsid_fd, 0);//将文件大小改为参数length 指定的大小
        if (ret < 0)
        {
            ret = -errno;
    ...
    // version stamp --------------对应store_version，不知道为什么为空
    ...
    ret = ::fstatfs(basedir_fd, &basefs); //----------查看/mnt/osd0目录的状态信息
    if (ret < 0)
    {
        ret = -errno;
    ...
    create_backend(basefs.f_type); /* f_type文件系统类型，初始化FileStore类成员FileStoreBackend * */

    ret = backend->create_current();//创建current目录
    if (ret < 0)
    {
    ...
    // write initial op_seq
    {
        ...
        if (fd < 0)
        {
            derr << "mkfs: failed to create " << current_op_seq_fn << ": "//对应current目录下的commit_op_seq
    ...
    ret = KeyValueDB::test_init(superblock.omap_backend, omap_dir);//omap_dir为/mnt/osd0/current/omap/
    if (ret < 0)  //调用LevelDBStore::_test_init创建levelDB数据库对应的文件
    {
...
int FileStore::mkjournal()
{
    ...
    open_journal();//实例化了一个FileJournal对象，并赋值给FileStore的父类JournalingObjectStore成员 *journal
    if (journal)
    {
        ret = journal->check();//FileJournal::check
        if (ret < 0)
        {
            ret = journal->create();
            if (ret)
...
int FileStore::_detect_fs()
{
    ...
    blk_size = st.f_bsize;/* 经过优化的传输块大小  */

    create_backend(st.f_type);//在mkfs中也调用了，为什么同一个流程里调两次
...
{
    bufferlist bl;
    ::encode(target_version, bl); //将target_version编码到bl中
...
int FileStore::mount()
{
    ...
    uint32_t version_stamp;// store_version
    ...
    if (!(generic_flags & SKIP_MOUNT_OMAP))
    {
        KeyValueDB * omap_store = KeyValueDB::create(g_ceph_context,
                                  superblock.omap_backend,// superblock.omap_backend为string类型，默认值为/leveldb
                                  omap_dir);
        if (omap_store == NULL)
        {
        ...
        omap_store->init();//调用LevelDBStore::init()

        stringstream err;
        if (omap_store->create_and_open(err))  //调用LevelDBStore::do_open
        {
            delete omap_store;
...
{
    // queue op on sequencer, then queue sequencer(序列) for the threadpool,
    // so that regardless of which order the threads pick up the
    // sequencer, the op order will be preserved.

    osr->queue(o);//OpSequencer有成员list<Op*> q，通过push_back将o加入q
    ...
    dout(5) << "queue_op " << o << " seq " << o->op
            ...
            << dendl;
    op_wq.queue(osr);//FileStore的成员op_wq，加入到op_wq的成员FileStore的成员deque<OpSequencer*> op_queue中
}
...
void FileStore::_finish_op(OpSequencer *osr)//当完成写任务后开始进行逐级回调
{
    list<Context*> to_queue;
    Op *o = osr->dequeue(&to_queue);//这个Op对象是在queue_op中放进去的
    ...
    {
        o->onreadable_sync->complete(0);//zhangmin add同步读的直接返回，直接执行complete函数
    }
    if (o->onreadable)  //回调处理函数，调用C_OSD_OnOpApplied
    {
        op_finisher.queue(o->onreadable); //异步的回调由op_finisher中的线程来完成
    }
    if (!to_queue.empty())
    {
...
{
    Context *onreadable;//对应applied
    Context *ondisk;    //对应submit
    Context *onreadable_sync;//对应applied_sync
    ObjectStore::Transaction::collect_contexts(
        tls, &onreadable, &ondisk, &onreadable_sync);//从list<Transaction>中的transaction中的三个list<Context *>成员中取出Context对象
    ...
    // set up the sequencer -------- 建立操作队列
    OpSequencer *osr;
    if (!posr)
        posr = &default_osr; //------ 这里如果posr为空，则将default_osr赋值给posr
    if (posr->p)
    {
        osr = static_cast<OpSequencer *>(posr->p);//rbd写分支
        dout(5) << "queue_transactions existing " << *osr << "/" << osr->parent << dendl; //<< " w/ q " << osr->q << dendl;
    }
    else
    {
        osr = new OpSequencer;
        osr->parent = posr;//parent为Sequencer
        posr->p = osr;
    ...
    //数据进行写入的时候需要写到journal中一份，当data数据失败的时候可以从journal中进行恢复
    if (journal && journal->is_writeable() && !m_filestore_journal_trailing)  //m_filestore_journal_trailing默认False
    {
        Op *o = build_op(tls, onreadable, onreadable_sync, osd_op);//根据对应的操作构造了一个Op的对象
        op_queue_reserve_throttle(o, handle);
        journal->throttle();


        uint64_t op_num = submit_manager.op_submit_start();//打印日志op_submit_start

        o->op = op_num;//这个Op的定义位于FileStore.h  对应op的序号op_seq
        ...
        if (m_filestore_journal_parallel)
        {
            ...
            //如果选择的文件系统（如brfs）时，这种文件系统存在checkpoint机制时，可以采用的方法，可以并行的处理日志与data的写入操作
            _op_journal_transactions(o->tls, o->op, ondisk, osd_op);

            // queue inside submit_manager op submission lock
            queue_op(osr, o);//将op放入op_wq队列中
        }
        else if (m_filestore_journal_writeahead)    //zhangmin add 对应filestore journal writeahead预写式日志
        {
            dout(5) << "queue_transactions (writeahead) " << o->op << " " << o->tls << dendl;
            //这种文件系统不存在checkpoint机制，所以必须先写入日志，等待日志完成后才可以写入data
            osr->queue_journal(o->op);//将op放入osr的list<uint64_t> jq中

            //用户IO操作到达FileStore以后，只要经过FileJournal存储后都可以立即回复给用户，无需等待操作正常落盘
            _op_journal_transactions(o->tls, o->op,//操作的seq序列号
                                     new C_JournaledAhead(this, osr, o, ondisk),
                                     osd_op);//转到FileJournal，写Journal并注册回调函数new C_JournaledAhead
        }
        else                   //Journal完成写入后进行回调将数据写入磁盘
        {
            assert(0);
        }
    ...
    if (!journal)  //没有journal日志，则直接入FileStore的队列
    {
        ...
        uint64_t op_num = submit_manager.op_submit_start();//++op_seq
        o->op = op_num;

        if (m_filestore_do_dump)
            dump_transactions(o->tls, o->op, osr);

        queue_op(osr, o);//直接加入FileStore的op_wq队列中，由op_tp线程池来处理
    ...
    int r = do_transactions(tls, op);//数据这里已经同步写入了磁盘了，而不是交由线程池异步处理的

    if (r >= 0)//如果数据写入成功，再写journal日志到磁盘，journal的写入仍然是放入writeq队列异步完成
    {
        _op_journal_transactions(tls, op, ondisk, osd_op);//这里传入的是一个commit(C_OSD_RepopCommit)的Context
    }
    else
    {
        delete ondisk;
    ...
    op_finisher.queue(onreadable, r);//zhangmin add 将读请求的Context回调加入Finisher的队列
...
//日志操作完成后，通过回调进入该函数，这个地方日志已经写完成了，将写操作加入op_wq的队列后，就开始通过两个Finisher进行回调向上返回
void FileStore::_journaled_ahead(OpSequencer *osr, Op *o, Context *ondisk)
{
    ...
    queue_op(osr, o);//入队列op_wq，然后由线程池调用_process(_do_op)和_process_finish(_finish_op)

    list<Context*> to_queue;
    //journal操作完成后，查找需要回调的操作，形成to_queue队列
    osr->dequeue_journal(&to_queue);//journal操作完成后，从list<uint64_t> jq移除一个操作项

    // do ondisk completions async, to prevent any onreadable_sync completions
    // getting blocked behind an ondisk completion.
    if (ondisk)  //zhangmin add 这里对应的处理是返回用户，ondisk不为空说明初始化了异步写磁盘的处理过程
    {
        dout(10) << " queueing ondisk " << ondisk << dendl;
        ondisk_finisher.queue(ondisk); //zhangmin add 将写请求的Context回调加入对应Finisher的队列
    }//------ rbd写流程分支，看日志打印，在写入日志打印之前，已经处理该回调了
    if (!to_queue.empty())  //检查Journal是否还存在等待回调的操作，若存在则交给ondisk_finisher
    {
        ondisk_finisher.queue(to_queue);//入队列的过程会唤醒finisher对应的处理线程
    }
}
    ...
    ThreadPool::TPHandle *handle)
{
    ...
    Transaction::iterator i = t.begin();//对应MemStore里也会调用，就是开始事物，实例化一个iterator对象
    ...
    while (i.have_op())
    {
        ...
        {
            coll_t cid = i.get_cid(op->cid);//解析文件目录
            ghobject_t oid = i.get_oid(op->oid);//解析文件的名字
            uint64_t off = op->off;
            uint64_t len = op->len;
            uint32_t fadvise_flags = i.get_fadvise_flags();
            bufferlist bl;
            i.decode_bl(bl);//解析要写入文件的数据
...
int FileStore::read(//read操作是同步完成然后返回到客户端的，不是异步，也不涉及事物的处理
    coll_t cid,
    const ghobject_t& oid,
    uint64_t offset,
    size_t len,
                      ...
                      const bufferlist& bl, uint32_t fadvise_flags)
{
    ...
    if (r >= 0 && m_filestore_sloppy_crc)//m_filestore_sloppy_crc默认为False
    {
    ...
    // flush?
    if (!replaying &&
            g_conf->filestore_wbthrottle_enable)//filestore_wbthrottle_enable默认为True
        wbthrottle.queue_wb(fd, oid, offset, len,
                            fadvise_flags & CEPH_OSD_OP_FLAG_FADVISE_DONTNEED);//写入后通过这种方式唤醒
    lfn_close(fd);
