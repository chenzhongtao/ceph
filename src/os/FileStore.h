...
private:
    ...
    string basedir, journalpath;  //# /tmp/osd /tmp/journal/journal
    osflagbits_t generic_flags;
    std::string current_fn;  //# basedir + "/current"
    std::string current_op_seq_fn; //# basedir + "/current/commit_op_seq"
    std::string omap_dir;    //# basedir + "/current/omap"
    uuid_d fsid;
    ...
    Finisher ondisk_finisher;  //# ondisk_finisher(g_ceph_context)
    ...
    list<Context*> sync_waiters; //#  存放每次sync之后的callback
    bool stop;  //# SyncThread 是否退出
    void sync_entry();
    ...
    Throttle throttle_ops, throttle_bytes;//# FileStore层的节流
    Finisher op_finisher;  //# 初始化 op_finisher(g_ceph_context)
...
private:
    ...
    double m_filestore_max_sync_interval;  //# 5
    double m_filestore_min_sync_interval;  //# 0.01
