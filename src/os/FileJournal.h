...
public:
    ...
    Mutex finisher_lock;
    Cond finisher_cond;
    uint64_t journaled_seq; //# 已经写入日志文件的seq
    bool plug_journal_completions; //# 塞住回调

    Mutex writeq_lock;
    Cond writeq_cond;
    deque<write_item> writeq; //# 写队列
    bool writeq_empty();
    ...
    struct header_t {
        ...
        int64_t start;      // offset of first entry  //# start 之前的日志是可以覆盖的日志  FileJournal::committed_thru会更新信息
        uint64_t committed_up_to; // committed up to  //# 日志以提交到哪个seq   FileJournal::prepare_header会更新信息
         ...
         */
        uint64_t start_seq; //# 开始写的seq     FileJournal::committed_thru会更新信息
    ...
    struct entry_header_t {
        ...
        uint64_t magic1;
        uint64_t magic2;
        //#日志的写入情况 header_len + pre_pad + data_len + post_pad + header_len
...
private:
    ...
    uint64_t last_committed_seq; //# 已经提交到磁盘的seq,日志可以清除, header.committed_up_to 写入日志的seq
    ...
    // in journal
    //# 准备写的日志队列  seq, queue_pos
    deque<pair<uint64_t, off64_t> > journalq;  // track seq offsets, so we can trim later.
    uint64_t writing_seq; //# 正在准备写入的seq


    // throttle
    Throttle throttle_ops, throttle_bytes; //# op数节流, 字节数节流
