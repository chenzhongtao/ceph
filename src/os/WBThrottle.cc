...
void WBThrottle::set_from_conf()
{
    ...
    if (fs == BTRFS) {
        ...
        size_limits.first =
            cct->_conf->filestore_wbthrottle_xfs_bytes_start_flusher; //# 41943040 40M
        size_limits.second =
            cct->_conf->filestore_wbthrottle_xfs_bytes_hard_limit;  //# 419430400  400M
        io_limits.first =
            cct->_conf->filestore_wbthrottle_xfs_ios_start_flusher; //# 500
        io_limits.second =
            cct->_conf->filestore_wbthrottle_xfs_ios_hard_limit;  //# 5000
        fd_limits.first =
            cct->_conf->filestore_wbthrottle_xfs_inodes_start_flusher; //# 500
        fd_limits.second =
            cct->_conf->filestore_wbthrottle_xfs_inodes_hard_limit;  //# 5000
    } else {
    ...
    boost::tuple<ghobject_t, FDRef, PendingWB> *next)
{
    assert(lock.is_locked());
    assert(next);
    while (!stopping && !beyond_limit()) //# 没有停止 并且没有那一项超出软限制
        cond.Wait(lock);
    if (stopping)
        return false;
