...
OSDService::OSDService(OSD *osd) :
    ...
    monc(osd->monc),
    op_wq(osd->op_shardedwq), //osd_op_tp
    peering_wq(osd->peering_wq),// op_tp
    recovery_wq(osd->recovery_wq),//recovery_tp
    snap_trim_wq(osd->snap_trim_wq),//disk_tp
    scrub_wq(osd->scrub_wq),//disk_tp
    rep_scrub_wq(osd->rep_scrub_wq),//disk_tp
...
void OSDService::queue_for_peering(PG *pg)
{
    ...
    peering_wq.queue(pg);//调用struct PeeringWQ中的_enqueue
              ...
              uuid_d fsid, int whoami)
{
    ...
    try
    {
        ...
        else
        {
            ...
            {
                dout(0) << " have superblock" << dendl;
                if (whoami != sb.whoami)   // ------ 这里很奇怪，sb没有被初始化，直接用其成员值来比较
                {
                    derr << "provided osd id " << whoami << " != superblock's " << sb.whoami << dendl;
                    ret = -EINVAL;
                    goto umount_store;
            ...
            else
            {
                ...
                sb.cluster_fsid = fsid; //集群的fsid-ceph_fsid
                sb.osd_fsid = store->get_fsid();//osd自身的fsid
                sb.whoami = whoami;
                ...
                ret = store->apply_transaction(t);//为什么不能直接想write_meta那样写入，而用transaction的方式
                if (ret)
                {
...
// cons/des
//zhangmin add  在ceph_osd.cc的main函数中，new一个OSD的对象
OSD::OSD(CephContext *cct_, ObjectStore *store_,
    ...
    osd_tp(cct, "OSD::osd_tp", cct->_conf->osd_op_threads, "osd_op_threads"),//调用threadpool构造函数初始化线程池
    osd_op_tp(cct, "OSD::osd_op_tp", //ShardedThreadPool 构造函数3个参数
              cct->_conf->osd_op_num_threads_per_shard * cct->_conf->osd_op_num_shards),//默认值为2*5
    ...
    command_tp(cct, "OSD::command_tp", 1),  //初始化了threadpool对象
    ...
    heartbeat_thread(this),//zhangmin add 这里又对OSD类中的成员对象heartbeat_thread进行了初始化
    heartbeat_dispatcher(this),//zhangmin add 这里又对OSD类中的成员对象heartbeat_dispatcher进行了初始化
    finished_lock("OSD::finished_lock"),
    op_tracker(cct, cct->_conf->osd_enable_op_tracker,
               cct->_conf->osd_num_op_tracker_shard),
    test_ops_hook(NULL),
    op_shardedwq(cct->_conf->osd_op_num_shards, this, //调用ShardedOpWQ构造函数
                 cct->_conf->osd_op_thread_timeout, &osd_op_tp),
    peering_wq(this, cct->_conf->osd_op_thread_timeout, &osd_tp),//用线程池对象初始化工作队列workqueue对象
    ...
    command_wq(this, cct->_conf->osd_command_thread_timeout, &command_tp),//用线程池对象初始化工作队列WorkQueue对象
...
int OSD::init()
{
    ...
    cluster_messenger->add_dispatcher_head(this);
        ...
        // i'm ready!
        client_messenger->add_dispatcher_head(this);//zhangmin add OSD类自身继承了Dispatcher类
    cluster_messenger->add_dispatcher_head(this);
              ...
              spg_t pgid, bool no_lockdep_check)
{
        ...
        PG* pg = _make_pg(createmap, pgid);
    {
        ...
        pg_map[pgid] = pg;//将创建的pg保存到osd的成员pg_map中,关联容器通过"[]"运算符插入新元素、修改或查询已有元素的附加数据
    ...
    spg_t pgid)
{
    ...
    else//这个PG的具体实现方式就是ReplicatedPG
        assert(0);
...
PG *OSD::get_pg_or_queue_for_pg(const spg_t& pgid, OpRequestRef& op)
{
    ...
    RWLock::RLocker l(pg_map_lock);
    //# pg_map中查找该pg
    ceph::unordered_map<spg_t, PG*>::iterator i = pg_map.find(pgid);//根据pg_id在osd->pg_map中查找pg
    if (i == pg_map.end())
        session->waiting_for_pg[pgid];//waiting_for_pg是Session的一个成员
...
bool OSD::_have_pg(spg_t pgid)
{
        ...
        assert(osd_lock.is_locked());
    RWLock::RLocker l(pg_map_lock);
    return pg_map.count(pgid);//返回容器pg_map中键值为pgid的个数
          ...
          PG::CephPeeringEvtRef evt)
{
    if (service.splitting(pgid))
    {
        ...
        return;
            ...
            if (!_have_pg(pgid))  //如果返回0，则说明PG不存在，取反则进入创建PG
        {
            // same primary?
            ...
            case RES_NONE:
            {
                ...
                PG::_init(*rctx.transaction, pgid, pp);
                    ...
                    PG *pg = _create_lock_pg(
                                 ...
                                 history, pi,
                                 *rctx.transaction);
                pg->handle_create(&rctx);//创建PG后，要对PG发送事件，事件在这里创建的
                pg->write_if_dirty(*rctx.transaction);
                ...
                dout(10) << *pg << " is new" << dendl;
                    ...
                    pg->queue_peering_event(evt);//处理其它发送过来的事件
                pg->unlock();
        ...
        else    //对应的PG已经存在
    {
        ...
        pg->queue_peering_event(evt);//PG处理其它节点发送过来的事件
        pg->unlock();
        return;
    }
...
bool OSD::ms_dispatch(Message *m)
{
    ...
    osd_lock.Lock();//--zhangmin add这里必须加锁，因为这个接口可能同时被两个线程调用
    if (is_stopping())
    {
        osd_lock.Unlock();
        m->put();
...
void OSD::ms_fast_dispatch(Message *m)
{
    ...
    OpRequestRef op = op_tracker.create_request<OpRequest>(m);//以message消息初始化了一个OpRequestRef对象
    {
        if (service.is_stopping())
        {
            m->put();
    ...
    void OSD::dispatch_op(OpRequestRef op)
    {
        ...
        case MSG_OSD_PG_QUERY:
            handle_pg_query(op);
            break;
        case MSG_OSD_PG_LOG://auth_osd收到发过来的包含g_query_t::LOG事件的消息
            handle_pg_log(op);
            break;
    ...
    bool OSD::dispatch_op_fast(OpRequestRef& op, OSDMapRef& osdmap)
    {
        ...
        switch(op->get_req()->get_type())
        {
            // client ops
        case CEPH_MSG_OSD_OP:
            handle_op(op, osdmap);//开始将这个处理由OSD转化为PG的处理
            break;
            // for replication etc.
        case MSG_OSD_SUBOP:
            ...
            break;
        case MSG_OSD_REPOP:
            handle_replica_op<MOSDRepOp, MSG_OSD_REPOP>(op, osdmap);//副本OSD收到这个请求进行处理，op是发送端封装好的op操作
            break;
        case MSG_OSD_SUBOPREPLY:
...
// pg creation
    ...
    {
        MOSDPGCreate *m = (MOSDPGCreate*)op->get_req();//从消息请求op中恢复出MOSDPGCreate 的message
                ...
                ++p, ++ci)
        {
            ...
            if (p->second.split_bits) // Skip split pgs
                continue;
            pg_t on = p->first;//解析出pg的编号pg_id到变量on上
            ...
            spg_t pgid;
            bool mapped = osdmap->get_primary_shard(on, &pgid);//根据pg_id的on开始创建出spg_t的pgid
            assert(mapped);//spg_t结构中主要记录两个信息，一个是pg_id的序号，另外一个就是primary的osd序号
            ...
            // register.
            creating_pgs[pgid].history = history;//将history信息根据pgid添加到creating_pgs结构中
            creating_pgs[pgid].parent = parent;//添加parent到creating_pgs结构中
            creating_pgs[pgid].acting.swap(acting);//添加acting到creating_pgs结构中
            calc_priors_during(pgid, created, history.same_interval_since,
                               creating_pgs[pgid].prior);//开始获取当前pg的所有osd信息 根据pgid添加到creating_pgs结构中
            ...
            if (can_create_pg(pgid))
            {
                ...
                creating_pgs.erase(pgid);
                pg->handle_create(&rctx);//开始一个pg的状态处理
                pg->write_if_dirty(*rctx.transaction);
...
// peering and recovery
                               ...
                               ThreadPool::TPHandle *handle)
    {
                ...
                is_active())
        {
            ...
            do_queries(*ctx.query_map, curmap);//这里传入的是RecoveryState中的成员boost::optional<RecoveryCtx> rctx的成员query_map
            do_infos(*ctx.info_map, curmap);//notify_list,query_map中的内容是通过函数send_query/send_notify放进去的。
        }
                         ...
                         OSDMapRef curmap)
    {
                ...
                ++pit)
        {
            ...
            int who = pit->first;//who代表要发过去的节点，就是和那个节点建立连接
        ...
        handle_pg_peering_evt(
            ...
            PG::CephPeeringEvtRef(
                new PG::CephPeeringEvt(
                    ...
                    PG::MLogRec(pg_shard_t(from, m->from), m)))//处理MOSDPGLog消息，解析出一个MLogRec事件，
        );//最后调用handle_peering_event处理改事件，完成状态切换

    }

    void OSD::handle_pg_info(OpRequestRef op)//
    {
        //当primary osd接收到MOSDPGInfo消息，解析为MInfoRec事件，这时primary osd的状态为active，接收MInfoRec开始处理
            ...
            handle_pg_peering_evt(
                ...
                PG::CephPeeringEvtRef(
                    new PG::CephPeeringEvt(
                        ...
                        PG::MInfoRec(//此时主OSD为active状态，进入状态机处理函数
                            pg_shard_t(
    ...
    void OSD::handle_pg_query(OpRequestRef op)
    {
        ...
        for (map<spg_t,pg_query_t>::iterator it = m->pg_list.begin();//begin取映射容器的第一个元素
                it != m->pg_list.end();
                ++it)
        {
            spg_t pgid = it->first;
                ...
                peering_wait_for_split[pgid].push_back(
                    PG::CephPeeringEvtRef(
                        new PG::CephPeeringEvt(
                            ...
                            PG::MQuery(pg_shard_t(from, it->second.from),//此处根据MOSDPGQuery消息构造事件MQuery& query
            ...
            }
            {
                ...
                if (pg_map.count(pgid))
                {
                    ...
                    pg->queue_query(//这个流程处理主OSD发过来的MOSDPGQuery消息，产生MQuery事件，触发PG状态机转换
                        it->second.epoch_sent, it->second.epoch_sent,
                        pg_shard_t(from, it->second.from), it->second);
                    pg->unlock();//it->second.from对应的是pg_query_t的成员shard_id_t from;
                    continue;
                }
            }//如果pg不存在，才执行下面的流程
            ...
            if (!osdmap->have_pg_pool(pgid.pool()))//pgid对应的存储池不在osdmap中
                continue;
                    ...
                    it->second.type == pg_query_t::FULLLOG)
            {
                ...
                if (con)
                {
                    MOSDPGLog *mlog = new MOSDPGLog(//这里初步理解是针对要删除的PG的处理，属于特殊情况
                        it->second.from, it->second.to,//但是对于pg_query_t::LOG类型还是需要向主节点返回MOSDPGLog消息
                        osdmap->get_epoch(), empty,
                        it->second.epoch_sent);
                    service.share_map_peer(from, con.get(), osdmap);
                    con->send_message(mlog);//MOSDPGLog消息发送到目标auth osd
                }
            }
            else
            {
                notify_list[from].push_back(//向容器尾部插入元素t
                    make_pair(
                        pg_notify_t(//
                            it->second.from, it->second.to,
                            it->second.epoch_sent,
                            osdmap->get_epoch(),
                            empty),
                        pg_interval_map_t()));//初步理解为PG删除时的特殊流程，对于pg_query_t::INFO类型，还是需返回MOSDPGNotify消息
            }
...
// RECOVERY
        ...
        void OSD::do_recovery(PG *pg, ThreadPool::TPHandle &handle)
        {
            ...
            else
            {
                ...
                int started;
                bool more = pg->start_recovery_ops(max, &rctx, handle, &started);//判断这个pg已经数据恢复完成的时候
...
// OPS
        ...
        void OSD::handle_op(OpRequestRef& op, OSDMapRef& osdmap)
        {
            ...
            MOSDOp *m = static_cast<MOSDOp*>(op->get_req());//op->get_req()返回的是OpRequestRef中的成员Message *request
            assert(m->get_type() == CEPH_MSG_OSD_OP);//如果为False，则assert跳出
            ...
            m->clear_payload(); //# bufferlist payload;  // "front" unaligned blob 解码后payload就没作用了
            ...
            // blacklisted?
            if (osdmap->is_blacklisted(m->get_source_addr())) //# OSDMap.cc 判断是否在黑名单
            {
            ...
            Session *client_session =//RefCountedObject
            ...
            if (op->rmw_flags == 0)
            {
                ...
                // set up a map send if the Op gets blocked for some reason
                /*#$1 = {osd = 0x5020000, name = {_type = 8 '\b', _num = 144130, static TYPE_MON = 1, static TYPE_MDS = 2, static TYPE_OSD = 4, static TYPE_CLIENT = 8, static NEW = -1}, con = {
                px = 0x546bb20}, osdmap = std::tr1::shared_ptr (count 175) 0x54321c0, map_epoch = 301, should_send = true}*/
                send_map_on_destruct share_map(this, m, osdmap, m->get_map_epoch());
                Session *client_session =
                    static_cast<Session*>(m->get_connection()->get_priv()); //# Connection.h
                if (client_session)
                {
                    client_session->sent_epoch_lock.Lock();
                }
                ...
                // calc actual pgid
                pg_t _pgid = m->get_pg(); //# 消息传过来的pgid //返回的是MOSDOp中的pg_t pgid，在客户端那边计算出对应的pg后初始化的
                int64_t pool = _pgid.pool();//定义位于结构体struct pg_t中，返回pool id
                if ((m->get_flags() & CEPH_OSD_FLAG_PGOP) == 0 &&
                        osdmap->have_pg_pool(pool))
                    _pgid = osdmap->raw_pg_to_pg(_pgid); //哈希值求模->pg_num  //定义位于OSDMap.h，进一步调用pg_pool_t的raw_pg_to_pg得到物理pg


                spg_t pgid; //struct spg_t 位于osd_types.h
                if (!osdmap->get_primary_shard(_pgid, &pgid)) //# OSDMap.h:689
                {
                ...
                //# 获取一个pg
                PG *pg = get_pg_or_queue_for_pg(pgid, op);//根据pgid构造对应的PG对象
                if (pg)
                {
                    op->send_map_update = share_map.should_send;
                    op->sent_epoch = m->get_map_epoch();
                    //# pg和op添加到 osd->op_wq队列
                    enqueue_op(pg, op);//从OSD处理的message转化为了PG处理的op
            ...
            void OSD::handle_replica_op(OpRequestRef& op, OSDMapRef& osdmap)
            {
                ...
                T *m = static_cast<T *>(op->get_req());//获取对应的消息类型
                assert(m->get_type() == MSGTYPE);
            ...
            void OSD::ShardedOpWQ::_process(uint32_t thread_index, heartbeat_handle_d *hb )
            {
                ...
                pair<PGRef, OpRequestRef> item = sdata->pqueue.dequeue();//消息请求出队�
                ...
                *_dout << dendl;
                    ...
                    osd->dequeue_op(item.first, op, tp_handle);//item.first为PG对象
...
//zhangmin add 这里实现的_enqueue继承自ShardedWQ中,PG::queue_op中调用osd->op_wq.queue
//queue里调用下面的_enqueue，入的队列是pqueue是PrioritizedQueue类型
            ...
            void OSD::dequeue_op(
                ...
                PGRef pg, OpRequestRef op,//PGRef类型为智能数据类型，还是PG类型
                ThreadPool::TPHandle &handle)
            ...
            void OSD::process_peering_events(//OSD启动后对应的peer_wq的线程就已经开始循环运行，循环调用下面的函数
                const list<PG*> &pgs,
                ThreadPool::TPHandle &handle
            )
            {
                ...
                dispatch_context(rctx, 0, curmap, &handle);//send_query的消息是通过这里发送到其它节点的
                ...
                else
                {
                    ...
                    PG::CephPeeringEvtRef evt = pg->peering_queue.front();
                    pg->peering_queue.pop_front();
                    pg->handle_peering_event(evt, &rctx); //处理peering_queue中保存的CephPeeringEvtRef事件，触发PG状态机的装换
                }
                need_up_thru = pg->need_up_thru || need_up_thru;
                same_interval_since = MAX(pg->info.history.same_interval_since,
                                          same_interval_since);
