...
public:
    ...
    */
    ...
    {
        assert(is_up(osd));
        //# get_addr OSDMap.h:464  entity_inst_t msg_type.h 371
...
public:
  ...
  pg_t raw_pg_to_pg(pg_t pg) const {
    assert(pools.count(pg.pool())); //------- 返回pg_t的成员 uint64_t m_pool
    return pools.find(pg.pool())->second.raw_pg_to_pg(pg);//从映射中找到对应的二元组，second表示第二个字段
  }//调用是pg_pool_t的raw_pg_to_pg，根据存储池和对象的hash值计算出pg的物理id
