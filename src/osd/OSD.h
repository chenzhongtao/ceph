...
class OSDService  //zhangmin add 在osd调用构造函数时，osd用this初始化了其成员OSDService service;
{

public:

    OSD *osd;//用创建的OSD类型的对象来初始化该成员
    CephContext *cct;
...
public:
    ...
    MonClient   *&monc;
    ShardedThreadPool::ShardedWQ < pair <PGRef, OpRequestRef> > &op_wq;//zhangmin add 使用osd->op_shardedwq初始化该成员
    ThreadPool::BatchWorkQueue<PG> &peering_wq;
    ThreadPool::WorkQueue<PG> &recovery_wq;
    ThreadPool::WorkQueue<PG> &snap_trim_wq;
    ThreadPool::WorkQueue<PG> &scrub_wq;
    ThreadPool::WorkQueue<MOSDRepScrub> &rep_scrub_wq;  //上面这个几个工作队列，用于存放不同各类型的消息
...
public:
    ...
    struct Session : public RefCountedObject  // Message也继承自RefCountedObject这个父类
    {
    ...
    map<spg_t, set<Session*> > session_waiting_for_pg;//关联容器的最大优势在于已经升序排列了，可以高效地根据键来查找容器中的一个元素。
...
private:
    ...
    // -- op tracking --
    OpTracker op_tracker;//调用OSD的构造函数会初始化该成员
    ...
    //zhangmin note: ShardedOpWQ继承ShardedThreadPool中的成员类ShardedWQ，对应OSDService中的op_wq成员
    class ShardedOpWQ: public ShardedThreadPool::ShardedWQ < pair <PGRef, OpRequestRef> >
    {
        ...
        struct ShardData
        {
            ...
            //映射类型，附加数据为pair类型的二元组pair<PGRef, OpRequestRef>，这样一个PG就对应一个list<OpRequestRef>，可保存多个请求
    ...
    public://pnum_shards默认值为5，ti默认值为15秒，调用父类ShardedThreadPool::ShardedWQ构造函数
            ...
            osd(o), num_shards(pnum_shards)
        {
            for(uint32_t i = 0; i < num_shards; i++)
            {
                ...
                shard_list.push_back(one_shard);//针对每个num_shards实例化一个ShardData对象，并压入shard_list中
            ...
            void _enqueue(pair <PGRef, OpRequestRef> item);//在父类中为纯虚函数
            ...
            op_shardedwq;//对应的处理线程为osd_op_tp
    ...
    protected:
        ...
        ceph::unordered_map<spg_t, PG*> pg_map; // protected by pg_map lock -- 使用[]增加元素，无需初始化
        //ceph::unordered_map对应map映射数据类型，key和value是一一对应的，哈希表是map类型，python中的字典也是这种类型
        //表示所有生活在当前OSD上的PG的数据成员是pg_map，完成PG的创建，加载，删除
        ...
        } recovery_wq;//对应的处理线程为recovery_tp


        void start_recovery_op(PG *pg, const hobject_t& soid);
        void finish_recovery_op(PG *pg, const hobject_t& soid, bool dequeue);
        void do_recovery(PG *pg, ThreadPool::TPHandle &handle);//恢复的核心函数
        bool _recover_now();
