...
struct pg_shard_t {//这个结构体当前的理解是对应一个OSD
  int32_t osd;
  shard_id_t shard;
...
struct object_locator_t {
  ...
  string nspace;    ///< namespace ---------------------这个指的是一个rbd映像(卷)吗?
...
// placement group id   pg_id对应pg在存储池中的物理序号

struct pg_t {
  uint64_t m_pool;//pool序号
  uint32_t m_seed;//pg序号
  int32_t m_preferred;
...
public:
    ...
    eversion_t(bufferlist& bl) : __pad(0)
    {
        decode(bl);
    }
...
struct pg_pool_t
{
    ...
    /*#
    某个pg_pool_t的内存值
    $3 = {flags = 1, type = 1 '\001', size = 1 '\001', min_size = 1 '\001', crush_ruleset = 0 '\000', object_hash = 2 '\002', pg_num = 1024,
    pgp_num = 1024, properties = std::map with 0 elements, erasure_code_profile = "", last_change = 12, last_force_op_resend = 0, snap_seq = {
      val = 0}, snap_epoch = 0, auid = 0, crash_replay_interval = 0, quota_max_bytes = 0, quota_max_objects = 0,
    snaps = std::map with 0 elements, removed_snaps = {_size = 0, m = std::map with 0 elements}, pg_num_mask = 1023, pgp_num_mask = 1023,
    tiers = std::set with 0 elements, tier_of = -1, read_tier = -1, write_tier = -1, cache_mode = pg_pool_t::CACHEMODE_NONE,
    target_max_bytes = 0, target_max_objects = 0, cache_target_dirty_ratio_micro = 400000, cache_target_full_ratio_micro = 800000,
    cache_min_flush_age = 0, cache_min_evict_age = 0, hit_set_params = {_vptr.Params = 0x7ffff351ca30, impl = {px = 0x0}}, hit_set_period = 0,
    hit_set_count = 0, min_read_recency_for_promote = 0, stripe_width = 0, expected_num_objects = 0}
    */
  ...
  __u8 object_hash;         ///< hash mapping object name to ps --  对应的hash算法
...
public:
   ...
   */
  pg_t raw_pg_to_pg(pg_t pg) const;//函数定义位于osd_types.cc中
...
public:
    ...
    uint64_t    objectno;  // (blockno / stripe_count ) 如果没有分条带就是 blockno ，oid名字的后12位数字
