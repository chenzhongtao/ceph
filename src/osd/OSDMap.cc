...
int OSDMap::object_locator_to_pg(
	...
	const object_t& oid,
	const object_locator_t& loc,//object_locator_t定义在osd_types.h中，其pool成员为int64_t
	pg_t &pg) const

{

    // calculate ps (placement seed)
    const pg_pool_t *pool = get_pg_pool(loc.get_pool());//返回的是object_locator_t的成员int64_t pool;
    if (!pool)
        return -ENOENT;
    ps_t ps;
    if (loc.hash >= 0)
    ...
    else
    {
        if (!loc.key.empty())
            ...
            ps = pool->hash_key(oid.name, loc.nspace);//用算法根据对象名称计算出hash值，这里nspace应该是卷名
    }
    // osd_types.h 274
    pg = pg_t(ps, loc.get_pool(), -1);//传入存储池id和根据对象名算出的hash值，hash值保存到pg_t的成员m_seed中
    return 0;
}
...
{
    // osd的数目(复制份数)是否可以改变 rep卷可以，ec不可以
...
{
    // map to osds[]
    //log: pps = 3645953053 根据m_pool和(m_seed得出的pg)再计算一个哈希值 32位
    ps_t pps = pool.raw_pg_to_pps(pg);  // placement ps --- 定义位于osd_types.cc，计算出了物理pg id但是没保存起来
    //log: 1 复制份数
    unsigned size = pool.get_size();


    // what crush rule?
    // CrushWrapper.h:987  ruleset=0, type=1, size=1
    int ruleno = crush->find_rule(pool.get_crush_ruleset(), pool.get_type(), size);//crush为ceph::shared_ptr<CrushWrapper>类型，为OSDMap类的成员
    // ruleono =0
    if (ruleno >= 0)
        //#  CrushWrapper.h:1020   ruleno =0, pps=3645953053 osd_weight={65536, 65536, 65536} 权重
        crush->do_rule(ruleno, pps, *osds, size, osd_weight);//crush算法，算出pg所在的osds


    _remove_nonexistent_osds(pool, *osds);

    //# 第一个非CRUSH_ITEM_NONE的节点为主复制节点
    *primary = -1;
    for (unsigned i = 0; i < osds->size(); ++i)
    {
                             ...
                             vector<int> *up, int *primary) const
{
    ...
    if (pool.can_shift_osds())
    {
        ...
        {
            // OSDMap::exists OSDMap.h:426 osd如何不存在或者down
                            ...
                            vector<int> *temp_pg, int *temp_primary) const
{
    ...
    if (p != pg_temp->end())
    {
        ...
        {
            if (!exists(p->second[i]) || is_down(p->second[i]))//id_down定义位于OSDMap.h的433行
            {
                if (pool.can_shift_osds())
                {
                    continue;
...
{
    const pg_pool_t *pool = get_pg_pool(pg.pool());//get_pg_pool定义位于OSDMap.h中，根据id返回一个pg_pool_t
    if (!pool)
    {
        if (up)
            up->clear();
    ...
    int _acting_primary;
    ps_t pps;
    //# raw = std::vector of length 1, capacity 1 = {2}   _up_primary =2 pps = 3645953053
    //# pg 对应哪个 osd 在raw为vector类型，一个pg可能对应多个 osd，_up_primary为主复制节点
    _pg_to_osds(*pool, pg, &raw, &_up_primary, &pps);//这个里面根据pg_t里面的hash值m_seed取摸运算得到物理的pg
    //# _up为所有up状态的节点， _up_primary为第一个up状态的节点
    _raw_to_up_osds(*pool, raw, &_up, &_up_primary);
    //# 有节点down时是否找其他节点代替(默认是直接返回)
    _apply_primary_affinity(pps, *pool, &_up, &_up_primary);
    //# 获取临时osd(不知什么时候用到，这里为空)
