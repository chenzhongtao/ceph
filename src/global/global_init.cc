                     ...
                     int flags)
{
    ...
    //# 参数初始化
    //# iparams  $4 = {module_type = 4 CEPH_ENTITY_TYPE_OSD , name = {type = 4, id = "0", type_id = "osd.0"}}
    //# iparams  $4 = {module_type = 8 CEPH_ENTITY_TYPE_CLIENT , name = {type = 8, id = "admin", type_id = "client.admin"}}
    CephInitParameters iparams = ceph_argparse_early_args(args, module_type, flags,
                                 &cluster, &conf_file_list);
    CephContext *cct = common_preinit(iparams, code_env, flags); // code_env = CODE_ENVIRONMENT_DAEMON
