...
void Pipe::reader()
{
            ...
            state != STATE_CONNECTING)
    {
        ...
        else if (tag == CEPH_MSGR_TAG_MSG)
        {
            ...
            //# 设置message的connection属性
            m->set_connection(connection_state.get());

            // note last received message. 
            in_seq = m->get_seq(); //# 1624
...
void Pipe::writer()
{
    ...
    {
        //# log: -- 191.168.45.104:0/1014331 >> 191.168.45.104:6805/9421 pipe(0x1a807e0 sd=5 :34172 s=2 pgs=10 cs=1 l=1 c=0x1a84a80).writer: state = open policy.server=0
                ...
                (is_queued() || in_seq > in_seq_acked))
        {
            ...
            if (m)
            {
                ...
                //# Connection.h:166    features = 70368744177663
                uint64_t features = connection_state->get_features();

                if (m->empty_payload())
                    //# log: -- 191.168.45.104:0/1014331 >> 191.168.45.104:6805/9421 pipe(0x1a807e0 sd=5 :34172 s=2 pgs=10 cs=1 l=1 c=0x1a84a80).writer encoding 6 features 70368744177663 0x1a8b970 osd_op(client.6010.0:6 rb.0.1011.6b8b4567.000000000000 [set-alloc-hint object_size 4194304 write_size 4194304,write 0~4194304] 1.6c591a2b ack+ondisk+write+known_if_redirected e220) v5
...
int Pipe::read_message(Message **pm, AuthSessionHandler* auth_handler)
{
    ...
    //# 读取front 和 middle(不知是干嘛的) 和 data
    //# 4194526
    ...
    // read data
    /*# 默认为小端，这里要转为cpu类型(大端或小端)
    前面front_len = header.front_len;为什么不用le32_to_cpu
    是不是默认转换
    */
    ...
    if (data_len)
    {
        ...
        while (left > 0)
        {
            ...
            // get a buffer
            //# connection_state为pipe所属的PipeConnection，继承Connection，有属性rx_buffers map<ceph_tid_t,pair<bufferlist,int> >
...
int Pipe::buffered_recv(char *buf, size_t len, int flags)
{
    ...
    //# 4096  如果长度小于4096 先读到buffer中，当如果每次len都小于4096且接近4096，会很影响cpu,有大量的memcpy操作
