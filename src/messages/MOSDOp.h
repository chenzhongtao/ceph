...
private:
    ...
    __u32 osdmap_epoch;  //# osdmap每次修改都会加1
    __u32 flags;
    utime_t mtime; //修改时间
    ...
    object_t oid;
    object_locator_t oloc; //# oloc = {pool = 7, key = "", nspace = "", hash = -1}
    pg_t pgid; //# pgid = {m_pool = 7, m_seed = 1713643959, m_preferred = -1}
public:
    vector<OSDOp> ops;
private:

    snapid_t snapid;  //# 18446744073709551614
    snapid_t snap_seq; //# 15
    vector<snapid_t> snaps;//# std::vector of length 2, capacity 2 = {{val = 15}, {val = 14}}
...
public:
    ...
    // marshalling
    //# 对MOSDOp类属性编码，结果放到  bufferlist payload; 
    virtual void encode_payload(uint64_t features)
    {
        ...
        {   
            ::encode(client_inc, payload); //#encoding.h:103 WRITE_INTTYPE_ENCODER(uint32_t, le32)
            ::encode(osdmap_epoch, payload); //# encoding.h:103
            ::encode(flags, payload);
            ::encode(mtime, payload);
            ::encode(reassert_version, payload);

            ::encode(oloc, payload);
            ::encode(pgid, payload); //# 结构体 osd_types.h 353  WRITE_CLASS_ENCODER(pg_t)
            ::encode(oid, payload);
