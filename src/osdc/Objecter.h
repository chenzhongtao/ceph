...
{
    // OSDOp  osd_types.h  3506
    vector<OSDOp> ops;
    int flags;
    int priority;

    //# 这个三个放的都是地址,一个ops对应一个，可能放的是Null
    vector<bufferlist*> out_bl;  //# & AioRequest  ceph::bufferlist m_read_data  // op为 CEPH_OSD_OP_READ 时会用到 // CEPH_OSD_OP_SPARSE_READ 有点区别 sparse_read
    vector<Context*> out_handler; //# & C_ObjectOperation_sparse_read   op为 CEPH_OSD_OP_SPARSE_READ 时会用到  
    vector<int*> out_rval;
    ...
    {
        OSDOp& osd_op = add_op(op);//返回类成员ops的一个元素的引用，后面修改元素的对应的参数
        osd_op.op.op = op;
    ...
    struct C_ObjectOperation_sparse_read : public Context
    {
        ...
        void finish(int r)
        {
            ...
            if (r >= 0)
            {
                try
                {
                    ::decode(*extents, iter); //# 本身extents是空的，根据osd返回的结果来解析(sparse是对osd而言的) ，客户端只传CEPH_OSD_OP_SPARSE_READ给osd
                    ::decode(*data_bl, iter);
                }
                catch (buffer::error& e)
                {
    ...
    {
        bufferlist bl;
        //CEPH_OSD_OP_SPARSE_READ使用宏定义的 __CEPH_FORALL_OSD_OPS(GENERATE_ENUM_ENTRY) rados.h
    ...
    void set_alloc_hint(uint64_t expected_object_size,
                        uint64_t expected_write_size )
    {
        // 这个宏在rados.h中定义 搜SETALLOCHINT可以搜到
...
private:
    atomic64_t last_tid;
    atomic_t inflight_ops; //# 正在发送中的op数
    atomic_t client_inc;  //# 
    uint64_t max_linger_id;
    atomic_t num_unacked;  //# 还没ack的数目
    atomic_t num_uncommitted; //# 还没committe的数目
...
public:
    ...
    struct Op : public RefCountedObject
    {
        ...
        uint64_t features;  // explicitly specified显式指定 op features 看Op构造函数

        vector<OSDOp> ops;

        snapid_t snapid;   //# 卷本身的snapid 18446744073709551614
        SnapContext snapc; //# {seq = {val = 15}, snaps = std::vector of length 2, capacity 2 = {{val = 15}, {val = 14}}}
        utime_t mtime;
...
public:
    ...
    {
        //# 新建Op 并交换 ObjectOperation& op vector<OSDOp> 两个回调相关的类作为Op的属性
                      ...
                      Context *onack, Context *oncommit, version_t *objver = NULL)
    {
        ...
        return op_submit(o);//将请求转化为Op请求，使用op_submit下发
    }
