...
int Objecter::_get_session(int osd, OSDSession **session, RWLock::Context& lc)
{
    ...
    //# 每个osd对应一个 OSDSession (回话)
    OSDSession *s = new OSDSession(cct, osd);
    osd_sessions[osd] = s;
    //# 每个OSDSession也就是每个osd 有自己的messenger
    //# OSDMap::get_inst OSDMap.h:480   SimpleMessenger::get_connection
...
ceph_tid_t Objecter::_op_submit(Op *op, RWLock::Context& lc)
{
  ...
  //通过计算当前object的保存的osd，然后将主osd保存在target中
    ...
    //# 根据osd获取一个OSDSession   属性ConnectionRef con 对SimpleMessenger中对应pipe的引用
    int r = _get_session(op->target.osd, &s, lc);//用来与主osd建立通信的
    if (r == -EAGAIN)
    {
        assert(s == NULL);
        lc.promote();
    ...
    // send?
    //# log: _op_submit oid rb.0.1011.6b8b4567.000000000000 @1 @1 [set-alloc-hint object_size 4194304 write_size 4194304,write 0~4194304] tid 0 osd.2
    ...
    {
        //# 新建一个消息类型
        m = _prepare_osd_op(op);//将op转化为MOSDop
    }
    ...
    //# 关联s 与 op   op->session = s; s->ops[op->tid] = op;  //map op放在s->ops中，回调时还会用到
    _session_op_assign(s, op);//将当前的op请求与这个session进行绑定，在后面发送请求的时候能知道使用哪一个session进行发送
    ...
    //# log: objecter 1 unacked, 1 uncommitted
...
int Objecter::_calc_target(op_target_t *t, bool any_change)
{
    ...
    int min_size = pi->min_size; // 1
    unsigned pg_num = pi->get_pg_num(); // 1024
    int up_primary, acting_primary;
    vector<int> up, acting;
    //#  up和acting一般是一样的(rebuild时可能不一样)  获取对应osds 和主复制节点
...
MOSDOp *Objecter::_prepare_osd_op(Op *op)
{
    ...
    m->ops = op->ops;//在客户端初始化消息
    m->set_mtime(op->mtime);
    m->set_retry_attempt(op->attempts++);//# 初始为0，设置一次就加1
...
void Objecter::_send_op(Op *op, MOSDOp *m)
{
  ...
  //调用SimpleMessager-> send_message(m), 再调用_send_message(),
  //再调用submit_message().在submit_message会找到之前的pipe，
  //然后调用pipe->send方法，最后通过pipe->writer的线程发送到目标osd
}
...
void Objecter::handle_osd_op_reply(MOSDOpReply *m)
{
            ...
            ++i, ++p, ++pb, ++pr, ++ph)
    {
        ...
        if (*pr)
            **pr = p->rval;
        //# 如果有handler  op=CEPH_OSD_OP_SPARSE_READ 见 sparse_read
        if (*ph)
        {
