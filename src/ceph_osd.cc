...
//# /usr/bin/ceph-osd -i 0 --pid-file /var/run/ceph/osd.0.pid -c /etc/ceph/ceph.conf --cluster ceph
int main(int argc, const char **argv)
{
    ...
    ObjectStore *store = ObjectStore::create(g_ceph_context,
                         g_conf->osd_objectstore, //# filestore
                         g_conf->osd_data,    //# /mnt/osd0
                         g_conf->osd_journal); //# /mnt/osd0/journal
    if (!store)
    {
    ...
    //cluster_fsid = {uuid = "\354W\365\305\355\256B/\244x\"I\\\037v&"} 
    //osd_fsid = {uuid = "\245\026\064\345\242\037J\260\231?\333#\267\231\023"}
    //w = 0
    ...
    Messenger *ms_objecter = Messenger::create(g_ceph_context, g_conf->ms_type,
                             ...
                             getpid());
    ms_cluster->set_cluster_protocol(CEPH_OSD_PROTOCOL); //集群内部协议
    ...
    if (g_conf->osd_heartbeat_use_min_delay_socket) //# false
    {
        ms_hbclient->set_socket_priority(SOCKET_PRIORITY_MIN_DELAY); //# 6
